...
warpmesh3::warpmesh3(const cyberreader &cr, int subsamplesize, const warpparms &params) :
			awarpmesh(cr,subsamplesize,params) {
	setdata();
}

warpmesh3::warpmesh3(istream &s, const warpparms &params, FP cs, const matrix &A, const vec &b) :
...
warpmesh3::~warpmesh3() {
	...
	for(i=0;i<DIM;i++) delete slack[i];
}

warpmesh3::warpmesh3(const trimesh &from, const warpparms &params, FP cs,
		const matrix &A, const vec &b) :
			awarpmesh(from,params,cs,A,b) {
	setdata();
}

void warpmesh3::addsprings(ilist<int> *springs) {
	...
	A = new smatrix(n+n+ttlsp+edges.length(),vertices.length(),3);
	cout << "matrix created" << endl;
	for(int i=0;i<DIM;i++) {
		b[i] = new svec(n+n+ttlsp+edges.length());
		slack[i] = new svec(n+n+ttlsp+edges.length());
	}
	...
	for(int i=0;i<edges.length();i++,c++) {
		in[0] = edges[i]->vertices[0]->index;
		in[1] = edges[i]->vertices[1]->index;
		val[0] = 1/(edges[i]->vertices[0]->edges.length()+
			edges[i]->vertices[1]->edges.length());
		val[1] = -val[0];
		A->setrow(c,2,in,val);
		for(d=0;d<DIM;d++) {
			b[d]->x[c] = 0;
	...
	if (c!=n+n+ttlsp+edges.length())
		cout << "problem setting up springs" << endl;
}

void warpmesh3::adduserspring(const vec &from, const vec &to) {
	FP alphas[3];
	face *f;
	vec t1,t2,p2;
	vertex *v1,*v2;
...
void warpmesh3::setsystem(trimesh *target) {
	...
	for(d=0;d<DIM;d++)
		for(i=0;i<slack[d]->s;i++)
			slack[d]->x[i] = 0;
}

void warpmesh3::iterate(trimesh *target) {
	...
	for(d=0;d<DIM;d++) {
		...
		solve(*A,*(b[d]),*(x[d]),w,*(slack[d]),momentum,
			0.1,
