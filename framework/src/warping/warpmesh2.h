 ...
 *implied warranty.
 */
#ifndef _WARPMESH2_H_
#define _WARPMESH2_H_
...
class warpmesh2 : public awarpmesh
{
public:
	// Load a new warpmesh from a cyberware scan
	// The subsamplesize refers to the compression per side needed --
	// see trimesh
	warpmesh2(const cyberreader &cr, int subsamplesize, const warpparms &params);
	...
	warpmesh2(istream &s, const warpparms &params, FP cs=1.0,
		const matrix &A = matrix::eye, const vec &b = vec::zero);

	warpmesh2(const trimesh &from, const warpparms &params, FP cs=1.0,
		const matrix &A = matrix::eye, const vec &b = vec::zero);
